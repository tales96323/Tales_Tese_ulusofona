...
A Figura~\ref{fig:req-flask} adota a mesma convenção visual dos
grafos direcionados do Capítulo~\ref{ch:fundamentos}: cada vértice
é identificado pelo respetivo rótulo \texttt{REQ\_*} e as arestas
usam setas escuras e proeminentes que terminam na borda de cada nó,
com curvatura que separa os pares recíprocos. Esta uniformização
torna o ciclo central imediatamente visível e facilita a leitura das
cadeias de dependência que atravessam os módulos.
...
\begin{itemize}
    \item \textbf{Validação do grafo concentrada num único passo
        subjetivo.} A correção do grafo é construída fase a fase
        (Secção~\ref{sec:ast-validacao}): a AST é determinística e
        garantida pela \emph{toolchain}; o \emph{Call Graph} e o Grafo
        de Requisitos são artefactos inspecionáveis
        (\code{call\_graph.png}, \code{req\_graph.json}). Assim, a
        validade do produto final fica circunscrita à correção de um
        único passo não automatizável --- o mapeamento
        \code{func\_to\_req}. Esta é uma força do desenho (o ponto de
        falha é isolado e auditável), mas também uma limitação: a
        verificação desse passo é, no presente trabalho, feita por
        inspeção do próprio investigador, sem confirmação independente
        (ver ponto seguinte).
